...
function LockedSection({ loginRedirect }: { loginRedirect: string }) {
  return (
    <div className="relative overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-paper)] p-8 text-center">
      ...
      <div className="relative">
        ...
        <p className="font-display text-lg font-bold text-[var(--color-ink)]">Full profile for members only</p>
...
export default function SeekerProfileView({ profile, isAuthenticated, isOwner, loginRedirect, visitors = [] }: Props) {
  const canViewFull = isAuthenticated || isOwner;
  ...
  return (
    <div className="card overflow-hidden">
      <div className="bg-gradient-to-br from-[var(--color-accent-soft)] to-white p-6 sm:p-8">
        <div className="flex items-start gap-4">
          {canViewFull && primaryPhoto ? (
            ...
            <div className="flex size-16 items-center justify-center rounded-full bg-gradient-to-br from-[var(--color-accent)] to-[#8b5cf6] text-2xl font-bold text-white">
              ?
            </div>
          )}
          <div className="min-w-0 flex-1">
            ...
            {canViewFull ? (
              ...
              <p className="mt-1 text-sm text-[var(--color-ink-muted)]">Berlin seeker · log in for details</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 border-t border-[var(--color-border)] p-6 sm:grid-cols-2 sm:p-8">
        <DetailRow label="Budget" value={<span className="font-display text-xl font-bold text-[var(--color-brand-deep)]">{formatBudget(publicProfile.budgetMin, publicProfile.budgetMax)}/mo</span>} />
      ...
      {canViewFull ? (
        ...
        <div className="border-t border-[var(--color-border)] p-6 sm:p-8">
          <LockedSection loginRedirect={loginRedirect} />
        </div>
      )}
    </div>
  );
